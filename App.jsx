...
// ── BOSS IMAGE MAP ────────────────────────────────────────
const BOSS_IMGS = {
sloth:   “3346886B-A7BD-484A-A7E1-F33F198B7F23.png”,
procras: “A46A22F7-4201-4ADE-B7C5-CC22C2A53B19.png”,
screen:  “871FA52C-6E9C-4144-8702-E0C27352AB24.png”,
doubt:   “45547F41-C3AE-4E67-ACC7-BE6674A0F501.png”,
sleep:   “A61B02A7-DC4F-4A2F-B637-934281A4C13D.png”,
compare: “492B8130-61E8-47BD-A30C-B930C8095F40.png”,
chaos:   “C4267998-3A78-4420-B740-0659129E5620.png”,
perfect: “8AEBD3F2-F434-4720-8B0B-C835239A292F.png”,
// Phobéos, Igris, Beru, Antares — à venir
};
...
animation: anim,
filter: isDead
? “drop-shadow(0 0 16px #39FF14) drop-shadow(0 0 32px #39FF1488) grayscale(0.3) brightness(1.2)”
: `drop-shadow(0 0 14px ${c}) drop-shadow(0 0 28px ${c}55)`,
mixBlendMode: “screen”,
borderRadius: 8,
overflow: “hidden”,
...
<style>{`@keyframes bossHit{0%,100%{transform:translateX(0)}20%{transform:translateX(-6px)}50%{transform:translateX(5px)}70%{transform:translateX(-3px)}} @keyframes playerHit{0%,100%{transform:translateX(0)}25%{transform:translateX(4px)}60%{transform:translateX(-4px)}} @keyframes fadeUp{0%{opacity:0;transform:translateY(8px)}100%{opacity:1;transform:translateY(0)}} @keyframes turnPulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
...
```
    ...
    <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:10}}>
      <div style={{position:"relative"}}>
        <BossSVG bossId={boss.id} color={bossWon?"#39FF14":boss.color} size={62} isShaking={bossShake} isDead={bossWon}/>
      </div>
      <div style={{flex:1}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
      <span style={{fontSize:7,color:"#111",fontFamily:"monospace"}}>PV BOSS</span>
      <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",fontWeight:700,color:bossWon?"#39FF14":bCol}}>{bossWon?"✦ VAINCU":Math.max(0,bossHp)+" / "+bossMaxHp}</span>
    </div>
    ...
    <div style={{background:"rgba(0,0,0,0.6)",border:`1px solid ${pCol}18`,borderRadius:12,
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:6}}>
        ...
        {(statusFx||[]).length>0&&(
          <div style={{display:"flex",gap:4,flexShrink:0}}>
            {statusFx.map((ef,i)=>(
              <div key={i} style={{padding:"2px 6px",borderRadius:6,background:`${ef.col}20`,border:`1px solid ${ef.col}40`,fontSize:6,color:ef.col,fontFamily:"monospace"}}>
                {ef.tag} {ef.turns}t
              </div>
            ))}
          </div>
    ...
    <div style={{display:"flex",gap:6,marginBottom:8,animation:"fadeUp 0.3s ease"}}>
      {lastPlayerAtk&&(
        <div style={{flex:1,background:"rgba(57,255,20,0.04)",border:"1px solid rgba(57,255,20,0.12)",borderRadius:9,padding:"6px 9px"}}>
          <div style={{fontSize:7,color:"#39FF14",fontFamily:"'Orbitron',monospace",marginBottom:1}}>TOI</div>
          <div style={{fontSize:8,color:"#C0C8E8",fontFamily:"monospace"}}>
      ...
      {lastBossAtk&&(
        <div style={{flex:1,background:`${boss.color}08`,border:`1px solid ${boss.color}18`,borderRadius:9,padding:"6px 9px"}}>
          <div style={{fontSize:7,color:boss.color,fontFamily:"'Orbitron',monospace",marginBottom:1}}>BOSS</div>
          <div style={{fontSize:8,color:"#C0C8E8",fontFamily:"monospace"}}>
  ...
  {!bossWon&&(
    <div>
      <div style={{fontSize:7,color:"#1A1A2A",fontFamily:"'Orbitron',monospace",letterSpacing:"0.12em",marginBottom:7}}>
          ...
          return (
            <button key={a.id} onClick={()=>!disabled&&onChoose(a)} disabled={disabled}
              ...
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:9,fontFamily:"'Orbitron',monospace",fontWeight:700,color:disabled?"#222":a.color,
                    whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{a.atkName}</div>
                  <div style={{fontSize:7,color:"#2A2A3A",fontFamily:"monospace"}}>{a.name}</div>
                </div>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:12,fontFamily:"'Orbitron',monospace",fontWeight:900,color:"#EF4444"}}>-{a.dmg}</span>
                {fx.type!=="none"&&(
                  <span style={{fontSize:6,padding:"2px 5px",borderRadius:4,
      ...
      {attacks.length>0&&isMyTurn&&(
        <button onClick={()=>onChoose(attacks[Math.floor(Math.random()*attacks.length)])}
          style={{width:"100%",marginTop:8,padding:"11px",background:"rgba(168,85,247,0.1)",
            border:"1px solid #A855F744",borderRadius:12,cursor:"pointer",fontSize:10,
...
```
    ...
    <button onClick={()=>onStart(workMins,breakMins)}
      style={{width:"100%",padding:"14px",background:"linear-gradient(135deg,rgba(239,68,68,0.25),rgba(239,68,68,0.08))",
        border:"1px solid #EF4444",borderRadius:13,cursor:"pointer",fontSize:11,
...
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:8,fontFamily:”‘Orbitron’,monospace”,fontWeight:700,color:col,marginBottom:2}}>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flexShrink:0}}>
              ...
              {onPomodoro&&(
                <button onPointerDown={e=>e.stopPropagation()}
                  ...
                  style={{display:"flex",alignItems:"center",gap:2,background:"rgba(239,68,68,0.06)",border:"1px solid rgba(239,68,68,0.15)",borderRadius:6,padding:"2px 6px",cursor:"pointer",fontSize:9}}>
                  ⏱{pomodoroLog?.[q.id]?<span style={{fontSize:6,color:"#EF444488",fontFamily:"monospace"}}>{Math.round((pomodoroLog[q.id]||0)/60)}m</span>:null}
                </button>
              )}
