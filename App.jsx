  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:8900,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.93)"}}>
      <div style={{textAlign:"center",maxWidth:285,width:"88%",animation:"rankReveal 0.6s ease"}}>
        <div style={{fontSize:7,color:"#39FF14",fontFamily:"'Orbitron',monospace",letterSpacing:"0.3em",marginBottom:12}}>⬛ SYSTÈME : VICTOIRE</div>
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"#000",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{position:"relative",zIndex:10,maxWidth:310,width:"90%",textAlign:"center",animation:"onboardIn 0.6s ease"}}>
        ...
        {s.btn&&<button onClick={()=>{if(step===2)onComplete(name);else setStep(p=>p+1);}} style={{width:"100%",padding:"14px",background:step===2?"linear-gradient(135deg,#A855F7,#7C3AED)":"rgba(168,85,247,0.12)",border:"1px solid #A855F7",borderRadius:12,cursor:"pointer",fontSize:13,fontFamily:"'Orbitron',monospace",fontWeight:900,color:step===2?"#fff":"#A855F7",letterSpacing:"0.12em"}}>{s.btn}</button>}
...
function DungeonEntry({ boss, rankData, onEnter }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:9800,background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
      ...
      {phase>=3&&(
        <button onClick={onEnter} style={{marginTop:26,padding:"13px 36px",background:`${c}18`,border:`1px solid ${c}`,borderRadius:12,cursor:"pointer",fontSize:12,fontFamily:"'Orbitron',monospace",fontWeight:700,color:c,letterSpacing:"0.14em",boxShadow:`0 0 20px ${c}44`,animation:"slideUp 0.4s ease"}}>
  ...
  return (
    <div style={{background:"rgba(5,0,10,0.98)",border:"1px solid rgba(168,85,247,0.15)",borderRadius:13,overflow:"hidden"}}>
      ...
      <div style={{padding:"9px 11px",display:"flex",flexDirection:"column",gap:8}}>
        ...
        <button onClick={save} style={{padding:"10px",background:"rgba(168,85,247,0.18)",border:"1px solid #A855F7",borderRadius:10,cursor:"pointer",fontSize:10,fontFamily:"'Orbitron',monospace",fontWeight:700,color:"#A855F7",letterSpacing:"0.1em"}}>{isNew?"✦ CRÉER":"✦ SAUVEGARDER"}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:8000,background:"rgba(0,0,0,0.97)",backdropFilter:"blur(6px)",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"10px"}}>
              ...
              return (
                <div key={sec} style={{marginBottom:12}}>
                  <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:5}}>
                    ...
                    <span style={{fontSize:7,fontFamily:"'Orbitron',monospace",color:sc,letterSpacing:"0.1em"}}>{sec}</span>
                    <span style={{fontSize:7,color:"#111",fontFamily:"monospace",marginLeft:"auto"}}>{sq.length}</span>
                  </div>
                  {sq.map(q=>{
                        ...
                        {isEd?<div style={{animation:"slideUp 0.2s ease"}}><QuestForm quest={q} onSave={f=>{onSave(f);setEditing(null);}} onCancel={()=>setEditing(null)}/></div>:(
                          <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(0,0,0,0.65)",border:`1px solid ${q.color}18`,borderRadius:10,padding:"7px 9px"}}>
                            ...
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:11,fontFamily:"'Rajdhani',sans-serif",fontWeight:600,color:"#B0B8D0",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{q.name}</div>
                              <div style={{display:"flex",gap:3,marginTop:1}}>
                                <span style={{fontSize:6,color:rc2.color,fontFamily:"'Orbitron',monospace",border:`1px solid ${rc2.color}33`,borderRadius:2,padding:"0 2px"}}>{q.rank}</span>
                                <span style={{fontSize:7,color:"#111",fontFamily:"monospace"}}>{q.duration}min · +{q.xp}XP</span>
                              </div>
                            </div>
                            <div style={{display:"flex",gap:4,flexShrink:0}}>
                              <button onClick={()=>setEditing(isEd?null:q)} style={{background:"rgba(168,85,247,0.08)",border:"1px solid rgba(168,85,247,0.18)",borderRadius:6,padding:"4px 7px",cursor:"pointer",fontSize:11,color:"#A855F7"}}>✏️</button>
                              <button onClick={()=>onDelete(q.id)} style={{background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.18)",borderRadius:6,padding:"4px 7px",cursor:"pointer",fontSize:11,color:"#EF4444"}}>🗑️</button>
                            </div>
                          </div>
                        )}
                      </div>
        ...
        {activeTab==="library"&&(
          <div>
            <div style={{fontSize:9,color:"#A855F7",fontFamily:"'Orbitron',monospace",letterSpacing:"0.15em",marginBottom:3}}>BIBLIOTHÈQUE DU SYSTÈME</div>
    ...
    <div style={{padding:"10px 14px 10px",background:`linear-gradient(180deg,${c}0C 0%,rgba(0,0,0,0.7) 100%)`,borderBottom:`1px solid ${c}18`,position:"relative",zIndex:10}}>
      <div style={{display:"flex",gap:11,alignItems:"center"}}>
        ...
        <div style={{flexShrink:0,width:52,height:52,borderRadius:14,background:`linear-gradient(135deg,${c}22,${c}0A)`,border:`2px solid ${c}66`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",boxShadow:`0 0 20px ${c}44,inset 0 1px 0 ${c}33`,position:"relative"}}>
          <div style={{fontSize:6,color:`${c}99`,fontFamily:"'Orbitron',monospace",fontWeight:700,letterSpacing:"0.1em"}}>RANG</div>
          <div style={{fontSize:22,fontFamily:"'Orbitron',monospace",fontWeight:900,color:c,textShadow:`0 0 12px ${c}`,lineHeight:1}}>{rankData.rank}</div>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:1}}>
            <div style={{fontSize:14,fontFamily:"'Orbitron',monospace",fontWeight:900,color:c,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",textShadow:`0 0 8px ${c}55`}}>{name||"CHASSEUR"}</div>
        ...
        <div style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
          ...
          <div style={{background:streak>0?"rgba(239,68,68,0.1)":"rgba(0,0,0,0.4)",border:`1px solid ${streak>0?"rgba(239,68,68,0.25)":"rgba(255,255,255,0.04)"}`,borderRadius:8,padding:"2px 6px",textAlign:"center",minWidth:32}}>
            ...
            <div style={{fontSize:8,fontWeight:900,color:streak>0?"#EF4444":"#111",fontFamily:"'Orbitron',monospace",lineHeight:1}}>{streak}</div>
          </div>
        </div>
      </div>
      ...
      {total>0&&(
        <div style={{marginTop:8,display:"flex",gap:2,alignItems:"center"}}>
          ...
          {done===total&&<span style={{fontSize:7,color:"#39FF14",fontFamily:"monospace",flexShrink:0}}>✦</span>}
        </div>
      )}
    </div>
  );
...
function TitlesTab({state,totalXp,streak,onActivate,activeTitle}) {
  ...
  return (
    <div style={{padding:"13px 12px 80px"}}>
      <div style={{fontSize:8,color:"#A855F7",fontFamily:"'Orbitron',monospace",letterSpacing:"0.2em",marginBottom:3}}>TITRES DU CHASSEUR</div>
      <div style={{fontSize:9,color:"#1A1A2A",fontFamily:"'Rajdhani',sans-serif",marginBottom:12}}>Débloque des titres et équipe-en un.</div>
      {unlocked.length===0&&<div style={{textAlign:"center",padding:"18px",color:"#111",fontSize:10,fontFamily:"'Rajdhani',sans-serif"}}>Aucun titre débloqué. Continue.</div>}
      {unlocked.map(t=>(
          ...
          {locked.map(t=>(
            <div key={t.id} style={{display:"flex",alignItems:"center",gap:11,background:"rgba(0,0,0,0.4)",border:"1px solid rgba(255,255,255,0.03)",borderRadius:13,padding:"9px 12px",marginBottom:5,opacity:0.35}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:7,color:"#111",fontFamily:"monospace"}}>{t.desc}</div>
              </div>
            </div>
          ))}
        </>
    ...
    <div style={{position:"fixed",inset:0,zIndex:8000,background:"rgba(0,0,0,0.97)",backdropFilter:"blur(6px)",display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"13px 14px",borderBottom:"1px solid rgba(255,215,0,0.1)",flexShrink:0,background:"rgba(0,0,0,0.8)"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5,background:"rgba(255,215,0,0.08)",border:"1px solid rgba(255,215,0,0.2)",borderRadius:10,padding:"5px 10px"}}>
          ...
          <span style={{fontSize:14,fontFamily:"'Orbitron',monospace",fontWeight:700,color:"#FFD700"}}>{gold}</span>
        </div>
      </div>
      ...
      {Object.keys(activeBoosts).some(k=>activeBoosts[k]>now) && (
        <div style={{padding:"8px 14px",borderBottom:"1px solid rgba(255,215,0,0.08)",background:"rgba(255,215,0,0.03)"}}>
          <div style={{fontSize:7,color:"#FFD70088",fontFamily:"'Orbitron',monospace",letterSpacing:"0.15em",marginBottom:5}}>BOOSTS ACTIFS</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {Object.entries(activeBoosts).map(([id,until])=>{
              if(until<=now) return null;
              const item=SHOP_ITEMS.find(i=>i.id===id);
              const left=Math.ceil((until-now)/60000);
              return item?<div key={id} style={{fontSize:9,fontFamily:"'Orbitron',monospace",color:"#FFD700",background:"rgba(255,215,0,0.1)",border:"1px solid rgba(255,215,0,0.25)",borderRadius:6,padding:"3px 7px"}}>
...
function Combat({boss,hp,maxHp,attacks,onAttack,particles,lastHit,isCrit,isDead,bossMsg,cooldowns,comboState,onComboTap}) {
  const now = Date.now();
  const pct = hp/maxHp;
  const hpCol = pct>0.5?"#EF4444":pct>0.25?"#F59E0B":"#FF3864";
  const {active:comboActive, windowMs, startedAt, bonus} = comboState||{};
  const comboProgress = comboActive ? Math.max(0,(windowMs-(Date.now()-startedAt))/windowMs) : 0;
  const inGreenZone = comboProgress>0.25 && comboProgress<0.65;

  return (
    <div style={{position:"relative"}}>
      {particles.map(p=>(
        <div key={p.id} style={{position:"absolute",left:`${p.x}%`,top:`${p.y}%`,zIndex:60,pointerEvents:"none",
          fontSize:p.combo?"22px":p.crit?"20px":"14px",fontFamily:"'Orbitron',monospace",fontWeight:900,
          color:p.combo?"#39FF14":p.crit?"#FFD700":boss.color,
          textShadow:`0 0 12px ${p.combo?"#39FF14":p.crit?"#FFD700":boss.color}`,
          animation:"dmgPop 0.85s ease forwards",whiteSpace:"nowrap"}}>
          {p.combo?"COMBO! ":p.crit?"CRIT! ":""}-{p.dmg}
        </div>
      ))}
      {bossMsg&&<div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",zIndex:50,background:"rgba(0,0,0,0.96)",border:`1px solid ${boss.color}55`,borderRadius:10,padding:"5px 12px",fontSize:10,fontFamily:"'Rajdhani',sans-serif",color:boss.color,whiteSpace:"nowrap",animation:"fadeIn 0.2s ease"}}>{bossMsg}</div>}

      {/* Boss panel */}
      <div style={{background:"rgba(0,0,0,0.65)",border:`1px solid ${isDead?"#39FF14":boss.color}30`,borderRadius:16,padding:"14px 12px",marginBottom:10,backdropFilter:"blur(8px)",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,background:`radial-gradient(ellipse at 50% 35%,${boss.color}06 0%,transparent 65%)`,pointerEvents:"none"}}/>
        {lastHit&&<div style={{position:"absolute",inset:0,zIndex:20,pointerEvents:"none",overflow:"hidden"}}>
          <div style={{position:"absolute",top:"46%",left:"50%",width:"160%",height:3,background:`linear-gradient(90deg,transparent,${isCrit?"#FFD700":boss.color},white,transparent)`,animation:"slashR 0.3s ease forwards",boxShadow:`0 0 14px ${isCrit?"#FFD700":boss.color}`}}/>
          <div style={{position:"absolute",top:"36%",left:"50%",width:"110%",height:2,background:`linear-gradient(90deg,transparent,${boss.color},transparent)`,animation:"slashD 0.3s ease forwards 0.04s"}}/>
          {isCrit&&<div style={{position:"absolute",inset:0,background:"rgba(255,215,0,0.08)",animation:"critFlash 0.4s ease"}}/>}
        </div>}
        <div style={{textAlign:"center",minHeight:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
          <BossSVG bossId={boss.id} color={isDead?"#39FF14":boss.color} size={70} isShaking={lastHit} isDead={isDead}/>
          <div style={{fontSize:7,color:"#333",fontFamily:"'Orbitron',monospace",letterSpacing:"0.18em",marginTop:6}}>{boss.title}</div>
          <div style={{fontSize:13,fontFamily:"'Orbitron',monospace",fontWeight:700,color:isDead?"#39FF14":boss.color,marginTop:1}}>{boss.name}</div>
          {!isDead&&<div style={{fontSize:8,color:"#1A1A2A",fontFamily:"monospace",marginTop:3,fontStyle:"italic",maxWidth:200,textAlign:"center",lineHeight:1.5}}>"{boss.lore}"</div>}
        </div>
        <div style={{marginTop:6}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
            <span style={{fontSize:7,color:"#111",fontFamily:"monospace"}}>PV DU BOSS</span>
            <span style={{fontSize:8,color:isDead?"#39FF14":hpCol,fontFamily:"'Orbitron',monospace",fontWeight:700}}>{isDead?"✦ VAINCU":Math.max(0,hp)+" / "+maxHp}</span>
          </div>
          <Bar v={isDead?0:hp} max={maxHp} color={isDead?"#39FF14":hpCol} h={9}/>
        </div>
      </div>

      {/* COMBO TIMING BAR */}
      {comboActive&&!isDead&&(
        <div style={{marginBottom:10,background:"rgba(0,0,0,0.8)",border:`1px solid ${inGreenZone?"#39FF14":"rgba(255,255,255,0.1)"}`,borderRadius:12,padding:"9px 12px",animation:inGreenZone?"comboFlash 0.4s ease-in-out infinite":undefined}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
            <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",color:inGreenZone?"#39FF14":"#F59E0B",fontWeight:700,letterSpacing:"0.08em"}}>
              {inGreenZone?"⚡ FRAPPE MAINTENANT!":"⏱ COMBO WINDOW"}
            </span>
            <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",color:"#555"}}>+{bonus}% DMG</span>
          </div>
          {/* Timing bar */}
          <div style={{position:"relative",height:14,background:"rgba(255,255,255,0.04)",borderRadius:7,overflow:"hidden",cursor:"pointer"}} onClick={onComboTap}>
            {/* Green zone indicator */}
            <div style={{position:"absolute",left:"25%",width:"40%",height:"100%",background:"rgba(57,255,20,0.15)",borderLeft:"1px solid #39FF1466",borderRight:"1px solid #39FF1466"}}/>
            {/* Sliding progress */}
            <div style={{position:"absolute",right:0,top:0,height:"100%",background:`linear-gradient(90deg,${inGreenZone?"#39FF14":"#F59E0B"},transparent)`,width:`${comboProgress*100}%`,borderRadius:7,transition:"width 0.1s linear",boxShadow:inGreenZone?"0 0 8px #39FF14":undefined}}/>
            {/* Tap cue */}
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",color:inGreenZone?"#39FF14":"#555",fontWeight:700}}>{inGreenZone?"TAP!":"..."}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {!isDead&&(
        <div>
          <div style={{fontSize:7,color:"#1A1A2A",fontFamily:"'Orbitron',monospace",letterSpacing:"0.12em",marginBottom:7}}>⚔️ ATTAQUES ({attacks.length})</div>
          {attacks.length===0&&<div style={{textAlign:"center",padding:"14px",color:"#111",fontSize:11,fontFamily:"'Rajdhani',sans-serif"}}>Complete des quêtes pour débloquer des attaques</div>}
          <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:175,overflowY:"auto"}}>
            {attacks.map(a=>{
              const cdUntil=cooldowns[a.id]||0;
              const onCD=cdUntil>now;
              const secLeft=onCD?Math.ceil((cdUntil-now)/1000):0;
              const cdTotal=a.cooldown||10;
              return (
                <button key={a.id} onClick={()=>!onCD&&onAttack(a)} disabled={onCD}
                  style={{display:"flex",alignItems:"center",gap:9,background:onCD?"rgba(0,0,0,0.4)":`${a.color}0A`,border:`1px solid ${onCD?"rgba(255,255,255,0.05)":a.color+"33"}`,borderRadius:10,padding:"8px 10px",cursor:onCD?"not-allowed":"pointer",textAlign:"left",opacity:onCD?0.45:1,position:"relative",overflow:"hidden",transition:"opacity 0.2s"}}>
                  {onCD&&<div style={{position:"absolute",left:0,top:0,height:"100%",background:"rgba(168,85,247,0.08)",width:`${(secLeft/cdTotal)*100}%`,transition:"width 1s linear",borderRight:`1px solid #A855F722`}}/>}
                  <span style={{fontSize:18,flexShrink:0}}>{a.emoji}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:10,fontFamily:"'Orbitron',monospace",fontWeight:700,color:onCD?"#333":a.color}}>{a.atkName}</div>
                    <div style={{fontSize:8,color:"#222",fontFamily:"'Rajdhani',sans-serif"}}>{a.name}</div>
                  </div>
                  <div style={{textAlign:"center",flexShrink:0,minWidth:28}}>
                    {onCD?<div style={{fontSize:10,fontFamily:"'Orbitron',monospace",color:"#333",animation:"cdPulse 1s ease-in-out infinite"}}>{secLeft}s</div>:(
                      <><div style={{fontSize:11,fontFamily:"'Orbitron',monospace",fontWeight:900,color:"#EF4444"}}>-{a.dmg}</div><div style={{fontSize:6,color:"#222",fontFamily:"monospace"}}>🩸</div></>
                    )}
                  </div>
                </button>
              );
          ...
          {attacks.length>0&&(
            <button onClick={()=>{const avail=attacks.filter(a=>!(cooldowns[a.id]>Date.now()));if(avail.length)onAttack(avail[Math.floor(Math.random()*avail.length)]);}}
              style={{width:"100%",marginTop:7,padding:"11px",background:"rgba(168,85,247,0.12)",border:"1px solid #A855F7",borderRadius:12,cursor:"pointer",fontSize:11,fontFamily:"'Orbitron',monospace",fontWeight:700,color:"#A855F7",letterSpacing:"0.1em"}}>
              ⚔️  FRAPPE RAPIDE
            </button>
          )}
        </div>
      ...
      {isDead&&(
        <div style={{textAlign:"center",padding:"12px",background:"rgba(57,255,20,0.04)",border:"1px solid rgba(57,255,20,0.12)",borderRadius:13}}>
...
const defaultState=()=>({
  ...
  weeklyProgress:{},   // {weekKey: {questId: done}}
  antaresHp:1500,      // monthly boss HP — decremented each active day
  antaresMonth:"",     // which month this HP belongs to
  generatedAtkNames:{},// {questId: "AI-generated name"}
  activeBet:null,      // {questId, amount, multiplier}
});
  ...
  return (
    <div style={{margin:"0 12px 14px",background:"rgba(0,0,0,0.7)",border:`1px solid ${color}22`,borderRadius:14,overflow:"hidden",backdropFilter:"blur(10px)"}}>
      <div style={{padding:"10px 14px 8px",borderBottom:`1px solid ${color}15`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:7,color:`${color}88`,fontFamily:"'Orbitron',monospace",letterSpacing:"0.2em"}}>DÉFIS DE LA SEMAINE</div>
          <div style={{fontSize:9,color:"#2A2A3A",fontFamily:"'Rajdhani',sans-serif",marginTop:1}}>{Object.values(done).filter(Boolean).length} / {weeklyQuests.length} accomplis</div>
        </div>
        ...
        {weeklyQuests.map(q=>{
          ...
          return (
            <div key={q.id} onClick={()=>onToggle(q.id)}
              ...
              <span style={{fontSize:17,flexShrink:0}}>{q.emoji}</span>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:12,fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:isDone?color:"#C0C8E0"}}>{q.name}</div>
                <div style={{fontSize:8,color:"#2A2A3A",fontFamily:"monospace",marginTop:1}}>{q.desc}</div>
              </div>
              <div style={{textAlign:"center",flexShrink:0}}>
                <div style={{fontSize:10,color:isDone?color:"#2A2A3A",fontWeight:900,fontFamily:"'Orbitron',monospace"}}>+{q.xp}</div>
                <div style={{fontSize:6,color:"#1A1A2A",fontFamily:"monospace"}}>XP</div>
                <div style={{fontSize:8,color:isDone?"#FFD700":"#1A1A2A",fontFamily:"monospace"}}>💰{q.gold}</div>
              </div>
            </div>
          );
        })}
  ...
  return (
    <div style={{margin:"0 12px 14px",background:`linear-gradient(135deg,rgba(192,38,211,0.06),rgba(0,0,0,0.85))`,border:`1px solid ${defeated?"#39FF14":alreadyHitToday?"rgba(192,38,211,0.15)":"rgba(192,38,211,0.3)"}`,borderRadius:16,overflow:"hidden",backdropFilter:"blur(12px)",position:"relative"}}>
      ...
      <div style={{padding:"12px 14px",position:"relative",zIndex:1}}>
        ...
        {defeated&&<div style={{textAlign:"center",padding:"6px",color:"#39FF14",fontSize:10,fontFamily:"'Orbitron',monospace",fontWeight:700}}>✦ MONARQUE ABATTU — VICTOIRE MENSUELLE</div>}
      </div>
    </div>
  );
}
    ...
    <div style={{position:"fixed",inset:0,zIndex:9200,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"rgba(8,8,12,0.98)",border:"1px solid rgba(255,215,0,0.25)",borderRadius:18,padding:"20px 18px",width:"88%",maxWidth:320,animation:"sysIn 0.3s ease"}}>
        <div style={{textAlign:"center",marginBottom:14}}>
          <div style={{fontSize:7,color:"#FFD70066",fontFamily:"'Orbitron',monospace",letterSpacing:"0.3em",marginBottom:6}}>⚠️ DOUBLE MISE</div>
          <div style={{fontSize:22,marginBottom:4}}>{quest.emoji}</div>
          <div style={{fontSize:13,fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#D0D8F0"}}>{quest.name}</div>
          <div style={{fontSize:9,color:"#2A2A3A",fontFamily:"monospace",marginTop:3}}>Miser du gold avant de cocher cette quête</div>
        </div>
        <div style={{background:"rgba(255,215,0,0.04)",border:"1px solid rgba(255,215,0,0.12)",borderRadius:12,padding:"14px",marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
            <span style={{fontSize:9,color:"#FFD70088",fontFamily:"'Orbitron',monospace"}}>MISE</span>
            <span style={{fontSize:12,color:"#FFD700",fontFamily:"'Orbitron',monospace",fontWeight:900}}>💰 {amount}g</span>
          </div>
          ...
          <div style={{display:"flex",gap:8}}>
            <div style={{flex:1,textAlign:"center",background:"rgba(57,255,20,0.06)",border:"1px solid rgba(57,255,20,0.15)",borderRadius:8,padding:"6px"}}>
              <div style={{fontSize:7,color:"#39FF1466",fontFamily:"monospace"}}>VICTOIRE</div>
              <div style={{fontSize:11,color:"#39FF14",fontFamily:"'Orbitron',monospace",fontWeight:700}}>+{amount*2}g</div>
            </div>
            <div style={{flex:1,textAlign:"center",background:"rgba(239,68,68,0.06)",border:"1px solid rgba(239,68,68,0.15)",borderRadius:8,padding:"6px"}}>
              <div style={{fontSize:7,color:"#EF444466",fontFamily:"monospace"}}>ÉCHEC (décoche)</div>
              <div style={{fontSize:11,color:"#EF4444",fontFamily:"'Orbitron',monospace",fontWeight:700}}>-{amount}g</div>
            </div>
          </div>
        </div>
        <div style={{display:"flex",gap:8}}>
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:9700,background:"rgba(0,0,0,0.97)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{textAlign:"center",maxWidth:300,width:"90%"}}>
        <div style={{fontSize:7,color:"#EF444466",fontFamily:"'Orbitron',monospace",letterSpacing:"0.4em",marginBottom:16}}>[ SYSTÈME ] CONTRE-ATTAQUE</div>
        ...
        {phase>=2&&<button onClick={onClose} style={{width:"100%",padding:"12px",background:"rgba(239,68,68,0.1)",border:"1px solid #EF4444",borderRadius:12,cursor:"pointer",fontSize:11,fontFamily:"'Orbitron',monospace",fontWeight:700,color:"#EF4444",letterSpacing:"0.1em",animation:"slideUp 0.3s ease"}}>
          ENCAISSER ›
        </button>}
      </div>
    </div>
...
function AwakenAttack({streak,onAttack,used}) {
  ...
  return (
    <button onClick={()=>!used&&onAttack()} disabled={used}
      ...
      <span style={{fontSize:22,animation:used?"none":"awakenPulse 2s ease-in-out infinite",flexShrink:0}}>⚡</span>
      <div style={{flex:1}}>
        <div style={{fontSize:11,fontFamily:"'Orbitron',monospace",fontWeight:900,color:used?"#444":"#FFD700",letterSpacing:"0.05em"}}>FORME ÉVEILLÉE</div>
        <div style={{fontSize:7,color:used?"#1A1A2A":"#6A5A20",fontFamily:"monospace"}}>{used?"Utilisée aujourd'hui":"Streak "+streak+"j · Une fois par jour"}</div>
      </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:9400,background:"rgba(0,0,0,0.97)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{maxWidth:300,width:"90%",textAlign:"center",animation:"bilanReveal 0.5s ease"}}>
        <div style={{fontSize:7,color:"#33334A",fontFamily:"'Orbitron',monospace",letterSpacing:"0.4em",marginBottom:18}}>[ SYSTÈME ] RAPPORT DE MISSION</div>
  ...
  return (
    <div style={{padding:"13px 12px 80px"}}>
      <div style={{background:"rgba(0,0,0,0.7)",border:"1px solid rgba(168,85,247,0.14)",borderRadius:14,padding:"13px",marginBottom:12,backdropFilter:"blur(8px)"}}>
        <div style={{fontSize:7,color:"#A855F7",fontFamily:"'Orbitron',monospace",letterSpacing:"0.18em",marginBottom:11}}>7 DERNIERS JOURS</div>
        ...
        <div style={{display:"flex",gap:4}}>{last7.map((d,i)=><div key={d.k} style={{flex:1,textAlign:"center",fontSize:6,color:i===6?"#A855F7":"#111",fontFamily:"monospace"}}>{d.day}</div>)}</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:12}}>
        {[{l:"XP TOTAL",v:totalXp.toLocaleString(),c:"#FFD700"},{l:"STREAK",v:`${streak}🔥`,c:"#EF4444"},{l:"JOURS ACTIFS",v:totalDays,c:"#39FF14"},{l:"GOLD",v:`💰 ${gold}`,c:"#F59E0B"},{l:"BOSS VAINCUS",v:defeatedBosses.length,c:"#A855F7"},{l:"QUÊTES",v:quests.length,c:"#60A5FA"}].map(k=>(
          <div key={k.l} style={{background:"rgba(0,0,0,0.65)",border:`1px solid ${k.c}18`,borderRadius:11,padding:"9px 11px",backdropFilter:"blur(6px)"}}>
            <div style={{fontSize:6,color:"#111",fontFamily:"'Orbitron',monospace",letterSpacing:"0.08em",marginBottom:2}}>{k.l}</div>
            <div style={{fontSize:17,fontFamily:"'Orbitron',monospace",fontWeight:700,color:k.c}}>{k.v}</div>
          </div>
        ))}
      </div>
      <div style={{background:"rgba(0,0,0,0.7)",border:"1px solid rgba(255,255,255,0.04)",borderRadius:14,padding:"13px",marginBottom:12,backdropFilter:"blur(8px)"}}>
        ...
        {sectionStats.map(({sec,pct,color})=>(
          <div key={sec} style={{marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
              <span style={{fontSize:9,fontFamily:"'Orbitron',monospace",color}}>{sec}</span>
              <span style={{fontSize:9,fontFamily:"'Orbitron',monospace",color,fontWeight:700}}>{Math.round(pct*100)}%</span>
            </div>
          ...
          {dayStats.map(({day,rate},i)=>{
            ...
            return(
              <div key={day} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                ...
                <div style={{fontSize:6,color:best?"#FFD700":"#111",fontFamily:"monospace"}}>{day}</div>
              </div>
            );
          })}
        </div>
...
function QuestSection({sec,sm,nd,sq,done,activeBet,gold,generatedAtkNames,onToggle,onBet,onReorder,rankData}) {
    ...
    <div style={{marginBottom:13}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",color:sm.c,letterSpacing:"0.12em",fontWeight:700}}>{sec}</span>
        </div>
        <span style={{fontSize:7,color:nd===sq.length&&sq.length>0?"#39FF14":"#1A1A2A",fontFamily:"monospace",fontWeight:nd===sq.length&&sq.length>0?700:400}}>
          ...
          return (
            <div key={q.id} data-cardidx={idx}
              ...
              <div
                ...
                <span style={{fontSize:17,flexShrink:0,filter:isDone?`drop-shadow(0 0 5px ${q.color})`:"none",transition:"filter 0.2s",lineHeight:1}}>{q.emoji}</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:isDone?q.color:"#D0D8F0",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{q.name}</div>
                  <div style={{display:"flex",gap:5,marginTop:2,alignItems:"center",flexWrap:"wrap"}}>
                    <span style={{fontSize:7,color:rc2.color,fontFamily:"'Orbitron',monospace",background:`${rc2.color}15`,border:`1px solid ${rc2.color}30`,borderRadius:4,padding:"1px 4px"}}>{q.rank}</span>
                    <span style={{fontSize:7,color:"#2A2A3A",fontFamily:"monospace"}}>⏱ {q.time} · {q.duration}min</span>
                    {hasBet&&<span style={{display:"flex",alignItems:"center",gap:2}}><SLIcon.gold size={11}/><span style={{fontSize:7,color:"#FFD700",fontFamily:"'Orbitron',monospace",fontWeight:700}}>{activeBet.amount}g</span></span>}
                  </div>
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flexShrink:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:3,background:isDone?`${q.color}18`:"rgba(0,0,0,0.5)",border:`1px solid ${isDone?q.color+"44":"rgba(168,85,247,0.08)"}`,borderRadius:9,padding:"3px 6px"}}>
                    ...
                    <span style={{fontSize:10,color:isDone?q.color:"#2A2A3A",fontWeight:900,fontFamily:"'Orbitron',monospace"}}>{q.xp}</span>
                  </div>
...
export default function App() {
  const [s,setS]=useState(()=>{try{const v=localStorage.getItem("arise_v11");return v?{...defaultState(),...JSON.parse(v)}:defaultState();}catch{return defaultState();}});
  ...
  const [genLoading,setGenLoading]=useState({});// {questId:true}
  ...
  const [comboState,setComboState]=useState({active:false,windowMs:2200,startedAt:0,bonus:50,lastAtkId:null});
  const [bossCounter,setBossCounter]=useState(null);  // {goldLost}
  ...
  useEffect(()=>{try{localStorage.setItem("arise_v11",JSON.stringify(s));}catch{}},[s]);
  ...
  const antaresHitKey="arise_v11_antares_"+today;
  ...
  // Morning dialog
  useEffect(()=>{
    const h=new Date().getHours(),k="arise_v11_dlg_"+today;
  ...
  useEffect(()=>{
    ...
    const penaltyKey="arise_v11_penalty_"+yKey;
  ...
  const bossCounterKey="arise_v11_counter_"+today;
  ...
  const awakenKey="arise_v11_awaken_"+today;
  ...
  const doAttack=useCallback(atk=>{
    if(bossWon||bossHp<=0)return;
    const crit=Math.random()<0.15;
    const variance=0.85+Math.random()*0.3;
    // Dynamic damage: rank of quest × streak multiplier, not fixed dmg value
    const dynBase=questDamage(atk, streak);
    const baseDmg=Math.round((crit?dynBase*2:dynBase)*variance);
    const isDiffAtk = comboState.lastAtkId && comboState.lastAtkId !== atk.id;
    const dmg = baseDmg;
    crit?snd.crit():snd.slash();
    setTimeout(()=>snd.hit(),80);
    setLastHit(true);setIsCrit(crit);setTimeout(()=>{setLastHit(false);setIsCrit(false);},700);
    const pid=Date.now();
    setParticles(p=>[...p,{id:pid,dmg,x:30+Math.random()*40,y:15+Math.random()*35,crit}]);
    setTimeout(()=>setParticles(p=>p.filter(x=>x.id!==pid)),1000);
    const msgs=todayBoss.atkMsg||[];
    if(msgs.length){setBossMsg(msgs[Math.floor(Math.random()*msgs.length)]);setTimeout(()=>setBossMsg(null),1800);}
    setFlashXp({xp:dmg,color:crit?"#FFD700":"#EF4444"});setTimeout(()=>setFlashXp(null),1000);
    const cdMs=(atk.cooldown||10)*1000;
    setCooldowns(p=>({...p,[atk.id]:Date.now()+cdMs}));
    // Open combo window if different attack used
    if(isDiffAtk){
      setComboState({active:true,windowMs:2200,startedAt:Date.now(),bonus:50,lastAtkId:atk.id});
      setTimeout(()=>setComboState(p=>({...p,active:false})),2200);
    } else {
      setComboState(p=>({...p,active:false,lastAtkId:atk.id}));
    }
    setS(p=>{
      const newHp=Math.max(0,(p.bossHpMap[today]??todayBoss.maxHp)-dmg);
      const won=newHp<=0&&!p.wonToday[today];
      if(won){
        setTimeout(()=>{setBossDead(true);snd.kill();setTimeout(()=>{setBossDead(false);setVictory({boss:todayBoss,gold:(todayBoss.maxHp/2)|0,xp:todayBoss.maxHp});},1100);},400);
        return{...p,bossHpMap:{...p.bossHpMap,[today]:0},wonToday:{...p.wonToday,[today]:true},defeatedBosses:p.defeatedBosses.includes(todayBoss.id)?p.defeatedBosses:[...p.defeatedBosses,todayBoss.id]};
      }
      return{...p,bossHpMap:{...p.bossHpMap,[today]:newHp}};
    });
  },[today,todayBoss,bossHp,bossWon,comboState]);

  const doComboTap=useCallback(()=>{
    if(!comboState.active||bossWon||bossHp<=0)return;
    const elapsed=Date.now()-comboState.startedAt;
    const progress=1-(elapsed/comboState.windowMs);
    const inZone=progress>0.25&&progress<0.65;
    setComboState(p=>({...p,active:false}));
    if(inZone){
      const comboDmg=Math.round(15+Math.random()*20+comboState.bonus/100*30);
      snd.crit();
      const pid=Date.now();
      setParticles(p=>[...p,{id:pid,dmg:comboDmg,x:35+Math.random()*30,y:10+Math.random()*25,crit:false,combo:true}]);
      setTimeout(()=>setParticles(p=>p.filter(x=>x.id!==pid)),1000);
      setFlashXp({xp:comboDmg,color:"#39FF14"});setTimeout(()=>setFlashXp(null),1000);
      toast({icon:"⚡",title:"COMBO !",desc:`+${comboDmg} dégâts bonus`,color:"#39FF14"});
      setS(p=>{
        const newHp=Math.max(0,(p.bossHpMap[today]??todayBoss.maxHp)-comboDmg);
        const won=newHp<=0&&!p.wonToday[today];
        if(won){setTimeout(()=>{setBossDead(true);snd.kill();setTimeout(()=>{setBossDead(false);setVictory({boss:todayBoss,gold:(todayBoss.maxHp/2)|0,xp:todayBoss.maxHp});},1100);},400);return{...p,bossHpMap:{...p.bossHpMap,[today]:0},wonToday:{...p.wonToday,[today]:true},defeatedBosses:p.defeatedBosses.includes(todayBoss.id)?p.defeatedBosses:[...p.defeatedBosses,todayBoss.id]};}
        return{...p,bossHpMap:{...p.bossHpMap,[today]:newHp}};
      });
    } else {
      toast({icon:"❌",title:"TROP TÔT/TARD",desc:"Rate la zone verte",color:"#EF4444"});
    }
  },[comboState,bossWon,bossHp,today,todayBoss]);
  ...
  return (
    <div style={{background:"#000",maxWidth:420,margin:"0 auto",fontFamily:"'Rajdhani',sans-serif",color:"#E0E8FF",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",height:"100vh"}}>
      ...
      {/* Flash XP */}
      {flashXp&&<div style={{position:"fixed",top:"22%",left:"50%",transform:"translateX(-50%)",zIndex:8900,pointerEvents:"none",fontSize:22,fontWeight:900,color:flashXp.color,fontFamily:"'Orbitron',monospace",animation:"xpPop 1.1s ease forwards",textShadow:`0 0 20px ${flashXp.color}`,whiteSpace:"nowrap"}}>{flashXp.xp>0?`+${flashXp.xp} XP`:""}</div>}
      ...
      <div style={{position:"relative",zIndex:10,flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"4px 13px 5px",borderBottom:`1px solid ${effectiveColor}10`,background:"rgba(0,0,0,0.4)"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:7,color:"#111",fontFamily:"'Orbitron',monospace",letterSpacing:"0.12em"}}>RESET DANS</span>
            <Countdown/>
          </div>
      ...
      <div style={{flex:1,overflowY:"auto",position:"relative",zIndex:10,display:isTablet?"flex":"block"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:11}}>
                <div style={{flex:1,background:"rgba(0,0,0,0.7)",border:`1px solid ${rankData.color}22`,borderRadius:12,padding:"9px 12px",backdropFilter:"blur(8px)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                    <span style={{fontSize:8,color:rankData.color,fontFamily:"'Orbitron',monospace",letterSpacing:"0.08em"}}>JOURNÉE</span>
                    <span style={{fontSize:9,color:"#FFD700",fontFamily:"'Orbitron',monospace",fontWeight:700}}>{xpToday}/{maxXpDay} XP</span>
                  </div>
                  <Bar v={xpToday} max={maxXpDay} color={xpToday>=maxXpDay?"#39FF14":rankData.color} h={6}/>
                </div>
                <button onClick={()=>setShowNotif(true)} style={{flexShrink:0,padding:"10px",background:"rgba(168,85,247,0.07)",border:"1px solid rgba(168,85,247,0.18)",borderRadius:12,cursor:"pointer",fontSize:14}}>🔔</button>
                <button onClick={()=>setShowMgr(true)} style={{flexShrink:0,padding:"10px 12px",background:"rgba(168,85,247,0.1)",border:"1px solid rgba(168,85,247,0.25)",borderRadius:12,cursor:"pointer",fontSize:10,fontFamily:"'Orbitron',monospace",color:"#A855F7",fontWeight:700}}>✏️</button>
              </div>
              {["AUBE","JOUR","MIDI","SOIR"].map(sec=>{
        ...
        {tab==="quests"&&(
          <div style={{padding:"10px 12px 80px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:11}}>
              ...
              <button onClick={()=>setShowNotif(true)} style={{flexShrink:0,padding:"10px 10px",background:"rgba(168,85,247,0.07)",border:"1px solid rgba(168,85,247,0.18)",borderRadius:12,cursor:"pointer",fontSize:14,backdropFilter:"blur(8px)"}}>🔔</button>
              <button onClick={()=>setShowMgr(true)} style={{flexShrink:0,padding:"10px 12px",background:"rgba(168,85,247,0.1)",border:"1px solid rgba(168,85,247,0.25)",borderRadius:12,cursor:"pointer",fontSize:10,fontFamily:"'Orbitron',monospace",color:"#A855F7",fontWeight:700,backdropFilter:"blur(8px)"}}>
                ✏️ GÉRER
              </button>
            </div>
          ...
          <div style={{padding:"0 0 80px",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{position:"relative",minHeight:180,background:`radial-gradient(ellipse at 50% 60%,${todayBoss.color}12 0%,rgba(0,0,0,0.95) 70%)`,borderBottom:`1px solid ${todayBoss.color}18`,overflow:"hidden",flexShrink:0}}>
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",paddingTop:18,paddingBottom:14,position:"relative",zIndex:2}}>
                <div style={{fontSize:7,color:`${todayBoss.color}66`,fontFamily:"'Orbitron',monospace",letterSpacing:"0.3em",marginBottom:6}}>
                ...
                <div style={{fontSize:8,color:`${bossWon?"#39FF14":todayBoss.color}99`,fontFamily:"'Orbitron',monospace",letterSpacing:"0.15em",marginTop:6}}>{todayBoss.title}</div>
                <div style={{fontSize:15,fontFamily:"'Orbitron',monospace",fontWeight:900,color:bossWon?"#39FF14":todayBoss.color,textShadow:`0 0 16px ${bossWon?"#39FF14":todayBoss.color}`,marginTop:2}}>{todayBoss.name}</div>
              </div>
              {/* HP bar full width */}
              {!bossWon&&(
              ...
              {/* Particles on boss hit */}
              {particles.map(p=>(
                <div key={p.id} style={{position:"absolute",left:`${p.x}%`,top:`${p.y}%`,zIndex:60,pointerEvents:"none",fontSize:p.combo?22:p.crit?20:15,fontFamily:"'Orbitron',monospace",fontWeight:900,color:p.combo?"#39FF14":p.crit?"#FFD700":todayBoss.color,textShadow:`0 0 14px ${p.combo?"#39FF14":p.crit?"#FFD700":todayBoss.color}`,animation:"dmgPop 0.9s ease forwards",whiteSpace:"nowrap"}}>
                  {p.combo?"COMBO! ":p.crit?"CRIT! ":""}-{p.dmg}
                </div>
              ))}
              {/* Hit flash overlays */}
              ...
              {bossMsg&&<div style={{position:"absolute",bottom:60,left:"50%",transform:"translateX(-50%)",zIndex:50,background:"rgba(0,0,0,0.94)",border:`1px solid ${todayBoss.color}44`,borderRadius:10,padding:"5px 14px",fontSize:10,fontFamily:"'Rajdhani',sans-serif",color:todayBoss.color,whiteSpace:"nowrap",animation:"fadeIn 0.2s ease"}}>{bossMsg}</div>}
              ...
              {!bossWon&&bossHp===todayBoss.maxHp&&(
                <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.7)",zIndex:30,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12,backdropFilter:"blur(2px)"}}>
                  <button onClick={()=>{setShowEntry(true);setTimeout(checkBossCounter,800);}}
                    style={{padding:"13px 32px",background:`${effectiveColor}18`,border:`2px solid ${effectiveColor}`,borderRadius:14,cursor:"pointer",fontSize:13,fontFamily:"'Orbitron',monospace",fontWeight:900,color:effectiveColor,letterSpacing:"0.12em",boxShadow:`0 0 30px ${effectiveColor}44`,animation:"sysIn 0.4s ease"}}>
                    🚪 ENTRER DANS LE DONJON
                  </button>
                </div>
              )}
            ...
            <div style={{padding:"12px 12px 0"}}>
              {(bossWon||bossHp<todayBoss.maxHp)&&(
                <button onClick={()=>setShowBilan(true)} style={{width:"100%",marginBottom:8,padding:"8px",background:"rgba(168,85,247,0.07)",border:"1px solid rgba(168,85,247,0.18)",borderRadius:10,cursor:"pointer",fontSize:9,fontFamily:"'Orbitron',monospace",fontWeight:700,color:"#A855F766",letterSpacing:"0.08em"}}>
                  📋 RAPPORT DE MISSION
                </button>
              )}
              {attacks.length===0&&!bossWon&&(
                <div style={{background:"rgba(239,68,68,0.04)",border:"1px solid rgba(239,68,68,0.1)",borderRadius:12,padding:"10px 12px",marginBottom:8,textAlign:"center"}}>
                  <div style={{fontSize:9,color:"#EF444488",fontFamily:"'Orbitron',monospace",fontWeight:700,marginBottom:2}}>⚠️ AUCUNE ATTAQUE DISPONIBLE</div>
                  <div style={{fontSize:10,color:"#2A2A3A",fontFamily:"'Rajdhani',sans-serif"}}>Complete des quêtes pour débloquer tes attaques.</div>
                </div>
              )}
              {!bossWon&&attacks.length>0&&<AwakenAttack streak={streak} onAttack={doAwakenAttack} used={awakenUsedToday}/>}
              {/* Combo timing bar */}
              {comboState.active&&!bossWon&&(
                <div style={{marginBottom:8,background:"rgba(0,0,0,0.8)",border:`1px solid ${comboState.active&&(1-(Date.now()-comboState.startedAt)/comboState.windowMs)>0.25&&(1-(Date.now()-comboState.startedAt)/comboState.windowMs)<0.65?"#39FF14":"rgba(255,255,255,0.1)"}`,borderRadius:12,padding:"9px 12px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                    <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",color:"#F59E0B",fontWeight:700}}>⏱ COMBO WINDOW</span>
                    <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",color:"#555"}}>+{comboState.bonus}% DMG</span>
                  </div>
                  <div style={{position:"relative",height:14,background:"rgba(255,255,255,0.04)",borderRadius:7,overflow:"hidden",cursor:"pointer"}} onClick={doComboTap}>
                    <div style={{position:"absolute",left:"25%",width:"40%",height:"100%",background:"rgba(57,255,20,0.12)",borderLeft:"1px solid #39FF1444",borderRight:"1px solid #39FF1444"}}/>
                    <div style={{position:"absolute",right:0,top:0,height:"100%",background:"linear-gradient(90deg,#F59E0B,transparent)",width:`${Math.max(0,1-(Date.now()-comboState.startedAt)/comboState.windowMs)*100}%`,borderRadius:7,transition:"width 0.1s linear"}}/>
                    <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                      <span style={{fontSize:8,fontFamily:"'Orbitron',monospace",color:"#555",fontWeight:700}}>TAP!</span>
                    </div>
                  </div>
                </div>
              )}
              {/* Attack list */}
              {!bossWon&&attacks.length>0&&(
                <div>
                  <div style={{fontSize:7,color:"#1A1A2A",fontFamily:"'Orbitron',monospace",letterSpacing:"0.12em",marginBottom:6}}>⚔️ ATTAQUES ({attacks.length})</div>
                  <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:160,overflowY:"auto"}}>
                    {attacks.map(a=>{
                      const cdUntil=cooldowns[a.id]||0;
                      const onCD=cdUntil>Date.now();
                      const secLeft=onCD?Math.ceil((cdUntil-Date.now())/1000):0;
                      const cdTotal=a.cooldown||10;
                      return (
                        <button key={a.id} onClick={()=>!onCD&&doAttack(a)} disabled={onCD}
                          style={{display:"flex",alignItems:"center",gap:9,background:onCD?"rgba(0,0,0,0.5)":`${a.color}0C`,border:`1px solid ${onCD?"rgba(255,255,255,0.04)":a.color+"30"}`,borderRadius:11,padding:"8px 10px",cursor:onCD?"not-allowed":"pointer",textAlign:"left",opacity:onCD?0.4:1,position:"relative",overflow:"hidden",boxShadow:onCD?"none":`0 2px 8px ${a.color}11`}}>
                          {onCD&&<div style={{position:"absolute",left:0,top:0,height:"100%",background:"rgba(168,85,247,0.06)",width:`${(secLeft/cdTotal)*100}%`,transition:"width 1s linear"}}/>}
                          <span style={{fontSize:18,flexShrink:0}}>{a.emoji}</span>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontSize:10,fontFamily:"'Orbitron',monospace",fontWeight:700,color:onCD?"#2A2A3A":a.color}}>
                              {s.generatedAtkNames?.[a.id]||a.atkName}
                              {genLoading[a.id]&&<span style={{fontSize:7,color:"#A855F766",marginLeft:4,animation:"cdPulse 1s infinite"}}>✦ IA...</span>}
                            </div>
                            <div style={{fontSize:8,color:"#2A2A3A",fontFamily:"'Rajdhani',sans-serif"}}>{a.name}</div>
                          </div>
                          <div style={{textAlign:"center",flexShrink:0,minWidth:30}}>
                            {onCD?<div style={{fontSize:10,fontFamily:"'Orbitron',monospace",color:"#2A2A3A",animation:"cdPulse 1s ease-in-out infinite"}}>{secLeft}s</div>:(
                              <><div style={{fontSize:12,fontFamily:"'Orbitron',monospace",fontWeight:900,color:"#EF4444"}}>-{a.dmg}</div><div style={{fontSize:6,color:"#222",fontFamily:"monospace"}}>🩸</div></>
                            )}
                          </div>
                        </button>
                      );
                  ...
                  <button onClick={()=>{const avail=attacks.filter(a=>!(cooldowns[a.id]>Date.now()));if(avail.length)doAttack(avail[Math.floor(Math.random()*avail.length)]);}}
                    style={{width:"100%",marginTop:7,padding:"12px",background:`${effectiveColor}10`,border:`1px solid ${effectiveColor}44`,borderRadius:12,cursor:"pointer",fontSize:12,fontFamily:"'Orbitron',monospace",fontWeight:700,color:effectiveColor,letterSpacing:"0.08em",boxShadow:`0 0 16px ${effectiveColor}22`}}>
                    ⚔️  FRAPPE RAPIDE
                  </button>
                </div>
              )}
              {bossWon&&<div style={{textAlign:"center",padding:"16px",background:"rgba(57,255,20,0.04)",border:"1px solid rgba(57,255,20,0.1)",borderRadius:13,marginTop:4}}>
                <div style={{fontSize:12,fontFamily:"'Orbitron',monospace",color:"#39FF14",fontWeight:700}}>✦ DONJON TERMINÉ</div>
                <div style={{fontSize:9,color:"#2A2A3A",fontFamily:"monospace",marginTop:4}}>Reviens demain pour un nouveau boss.</div>
              </div>}
            </div>
          </div>
        )}
        ...
        {tab==="weekly"&&(
          <div style={{padding:"12px 0 80px"}}>
            <div style={{padding:"0 14px 10px"}}>
              <div style={{fontSize:8,color:"#A855F7",fontFamily:"'Orbitron',monospace",letterSpacing:"0.2em",marginBottom:2}}>DÉFIS HEBDOMADAIRES</div>
              <div style={{fontSize:9,color:"#2A2A3A",fontFamily:"'Rajdhani',sans-serif"}}>Reset chaque lundi. XP et gold massifs.</div>
            </div>
            ...
            <div style={{padding:"0 14px 10px"}}>
              <div style={{fontSize:8,color:"#C026D3",fontFamily:"'Orbitron',monospace",letterSpacing:"0.2em",marginBottom:2}}>BOSS MENSUEL</div>
              <div style={{fontSize:9,color:"#2A2A3A",fontFamily:"'Rajdhani',sans-serif",marginBottom:8}}>Frappe Antares une fois par jour. Vaincs-le avant la fin du mois.</div>
            </div>
          ...
          <div style={{padding:"14px 12px 80px"}}>
            ...
            {Object.entries(s.activeBoosts||{}).some(([,v])=>v>Date.now())&&(
              <div style={{background:"rgba(255,215,0,0.04)",border:"1px solid rgba(255,215,0,0.12)",borderRadius:12,padding:"10px 12px",marginBottom:12}}>
                <div style={{fontSize:7,color:"#FFD70088",fontFamily:"'Orbitron',monospace",letterSpacing:"0.15em",marginBottom:6}}>⚡ BOOSTS ACTIFS</div>
        ...
        {tab==="profile"&&(
          <div style={{padding:"14px 12px 80px"}}>
            ...
            {(()=>{
              ...
              return (
                <div style={{background:"rgba(0,0,0,0.7)",border:`1px solid #EF444430`,borderRadius:14,padding:"12px 14px",marginBottom:12,backdropFilter:"blur(8px)"}}>
                  <div style={{fontSize:7,color:"#EF4444",fontFamily:"'Orbitron',monospace",letterSpacing:"0.15em",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
            ...
            {(()=>{
              ...
              return (
                <div style={{background:"rgba(0,0,0,0.7)",border:`1px solid rgba(168,85,247,${sc.all?"0.5":"0.15"})`,borderRadius:14,padding:"12px 14px",marginBottom:12,backdropFilter:"blur(8px)"}}>
                  <div style={{fontSize:7,color:"#A855F7",fontFamily:"'Orbitron',monospace",letterSpacing:"0.15em",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
            ...
            <button onClick={()=>{if(window.confirm("Réinitialiser l'identité?"))setS(p=>({...p,onboarded:false,name:""}));}} style={{width:"100%",padding:"9px",background:"rgba(239,68,68,0.04)",border:"1px solid rgba(239,68,68,0.1)",borderRadius:10,cursor:"pointer",fontSize:8,color:"#EF444455",fontFamily:"'Orbitron',monospace"}}>
              RÉINITIALISER L'IDENTITÉ
            </button>
            <button onClick={()=>setShowImportExport(true)} style={{width:"100%",marginTop:7,padding:"10px",background:"rgba(168,85,247,0.07)",border:"1px solid rgba(168,85,247,0.2)",borderRadius:10,cursor:"pointer",fontSize:9,fontFamily:"'Orbitron',monospace",fontWeight:700,color:"#A855F766",letterSpacing:"0.08em"}}>
              💾 IMPORT / EXPORT
            </button>
          </div>
        )}
          ...
          {TABS.map(t=>{
            ...
            return (
              <button key={t.id} onClick={()=>{setTab(t.id);snd.tap();}}
                ...
                <span style={{fontSize:7,fontFamily:"'Orbitron',monospace",letterSpacing:"0.04em",color:active?effectiveColor:"transparent",fontWeight:700,transition:"color 0.2s"}}>{t.label}</span>
              </button>
            );
          })}
        </div>
